...
export function renderLauncher(spec) {
  ...
  const lines = [
    ...
    'set "OWNER_PID="',
    '',
    'rem The path of this file, captured before the argument parser runs: `shift`',
    'rem moves the arguments down INCLUDING %0, so by the time the --silent branch',
    'rem needs its own path, %~f0 is the last flag that was parsed.',
    'set "SELF=%~f0"',
    '',
    'rem ---- run log: the console is gone the moment a window is closed, so the',
    'rem process and its exit code are also written where they can be read later.',
    'rem A log that cannot be created becomes NUL, which accepts writes silently',
    'rem rather than failing every line that follows.',
    'set "LOGDIR=%LOCALAPPDATA%\\dsh-launch"',
    'set "LOG=%LOGDIR%\\launcher.log"',
    'if not exist "%LOGDIR%" mkdir "%LOGDIR%" >nul 2>nul',
    'if not exist "%LOGDIR%" set "LOG=NUL"',
    '',
    'rem ---- settings handed over by a --silent re-launch of this same file ----',
    'if defined DSH_LAUNCH_HIDDEN (',
    '    if defined DSH_LAUNCH_PORT set "PORT=%DSH_LAUNCH_PORT%"',
    '    if defined DSH_LAUNCH_WORKDIR set "WORKDIR=%DSH_LAUNCH_WORKDIR%"',
    '    if defined DSH_LAUNCH_NOOPEN set "NO_OPEN=%DSH_LAUNCH_NOOPEN%"',
    '    if defined DSH_LAUNCH_DRYRUN set "DRY_RUN=%DSH_LAUNCH_DRYRUN%"',
    ')',
    '',
    'rem ---- command line ----',
    ':parse_args',
    ...
    'if "%SILENT%"=="1" if not defined DSH_LAUNCH_HIDDEN goto silent_relaunch',
    'goto after_silent',
    '',
    'rem A hidden run has no console to read and no key to press, so this file',
    'rem re-launches itself windowless and returns immediately. The settings it',
    'rem resolved travel in the environment, which is inherited across the handoff.',
    ':silent_relaunch',
    'set "DSH_LAUNCH_HIDDEN=1"',
    'set "DSH_LAUNCH_PORT=%PORT%"',
    'set "DSH_LAUNCH_WORKDIR=%WORKDIR%"',
    'set "DSH_LAUNCH_NOOPEN=%NO_OPEN%"',
    'set "DSH_LAUNCH_DRYRUN=%DRY_RUN%"',
    'rem The path travels in the environment, not inside the command text: cmd',
    'rem would end the -Command string at the first inner quote, and PowerShell',
    'rem binds trailing arguments to nothing under -Command (measured: $args[0]',
    'rem arrives empty and the path lands on the command line as literal text).',
    'set "DSH_LAUNCH_SELF=%SELF%"',
    'powershell -NoProfile -Command "Start-Process -FilePath $env:DSH_LAUNCH_SELF -WindowStyle Hidden" >nul 2>nul',
    'if errorlevel 1 goto silent_fallback',
    'call :log "handed off to a hidden copy"',
    'exit /b 0',
    '',
    'rem No PowerShell means no hidden window; a visible run beats no run.',
    ':silent_fallback',
    'set "DSH_LAUNCH_HIDDEN="',
    `echo ${m.silentFallback}`,
    'call :log "no powershell for a hidden launch; continuing with a console window"',
    '',
    ':after_silent',
    'call :log "--- %DATE% %TIME% %~nx0 port=%PORT% ---"',
    '',
    ...
    `echo ${m.launching}`,
    `call :log "run: ${launchCommand} %OPEN_ARG%"`,
    `call ${launchCommand} %OPEN_ARG%`,
    'set "CODE=%ERRORLEVEL%"',
    'call :log "harness exit code %CODE%"',
    ...
    ':port_dsh',
    'call :log "refused: port %PORT% already serves a harness instance"',
    `echo ${m.portDsh}`,
    `echo ${m.portDshHint1}`,
    `echo ${m.portDshHint2}`,
    'set "EXITCODE=2"',
    ...
    '',
    ':port_foreign',
    'call :log "refused: port %PORT% is held by another program"',
    `echo ${m.portForeign}`,
    ...
    `echo     PID: %OWNER_PID%`,
    'call :log "port owner pid %OWNER_PID%"',
    ':port_foreign_hint',
    ...
    '',
    ':err_no_node',
    'call :log "refused: node is not on PATH"',
    `echo ${m.nodeMissing}`,
    ...
    '',
    ':err_no_runner',
    'call :log "refused: %RUNNER% did not answer"',
    ...
    '',
    ':err_workdir',
    'call :log "refused: work directory %WORKDIR% is unusable"',
    ...
    '',
    ':err_probe',
    'call :log "refused: the port probe reached no verdict"',
    `echo ${m.probeFailed}`,
    ...
    '',
    ':err_server',
    'call :log "the harness exited with code %CODE%"',
    ...
    ':fail',
    'echo.',
    'call :log "exit %EXITCODE%"',
    `echo ${m.logHint}`,
    'if defined DSH_LAUNCH_HIDDEN (endlocal & exit /b %EXITCODE%)',
    ...
    '',
    'rem ---- run log ----',
    'rem Appends one line. Called with the redirect first so a message ending in a',
    'rem digit can never be read as a stream handle.',
    ':log',
    '>>"%LOG%" echo %~1',
    'exit /b 0',
    '',
